...
function getSumBetweenNumbers(n1, n2) {
  return Array(n2 - n1 + 1)
    .fill(n1)
    .reduce((res, val, index) => res + val + index, 0);
}
...
function doRectanglesOverlap(rect1, rect2) {
  const normalizeRect1 = { ...rect1, top: 0, left: 0 };
  const normalizeRect2 = {
    ...rect2,
    top: rect2.top - rect1.top,
    left: rect2.left - rect1.left,
  };

  const dWidth = normalizeRect1.width - normalizeRect2.left;
  const dHeight = normalizeRect1.height - normalizeRect2.top;
...
function findFirstSingleChar(str) {
  ...
  str.split('').forEach((char) => {
    ...
    if (charQuantity[index] === undefined) {
      charQuantity[index] = 1;
      return;
    }
    charQuantity[index] += 1;
  });

  return str[charQuantity.indexOf(1)];
}
...
function isCreditCardNumber(ccn) {
  return (
    ccn
      ...
      .map((value, index) => {
        if (index % 2 === 1) {
          let multiplication = Number(value) * 2;

          if (multiplication > 9) {
            multiplication = (multiplication % 10) + Math.trunc(multiplication / 10);
          }
...
function getDigitalRoot(num) {
  ...
  while (tr) {
    ...
    number = number
      .split('')
      .reduce((result, digit) => String(Number(result) + Number(digit)), '');
  }
...
function check(str, bracketsConfig) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (bracketsBuffer.length === 0 || leftBrackets.includes(str[i])) {
      ...
      const lastSymbol = bracketsBuffer.pop();
      if (
